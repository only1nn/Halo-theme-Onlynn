...
Onlynn 是一款基于 Astro 构建的 **Halo CMS 主题**。它先用 Astro 编写组件与模板，构建后输出为 Halo 使用的 **Thymeleaf 模板**，再由 Halo（Spring Boot + Thymeleaf）在服务端渲染最终页面。
...
- 静态资源用 `#theme.assets("/assets/...")` 或 `@{/assets/...}` 引用，构建后路径带 `/themes/onlynn` 前缀。
...
- **全局 i18n 助手**：`t()` 读取 `window.i18nResources` 的客户端翻译助手统一由 `Layout.astro` 注入（`window.__onlynnI18n`，另含 `__onlynnLangTag`/`__onlynnSetLanguage`/`__onlynnBigNum`）。内联脚本复用即可，不要各自复制实现。
...
- **文章页三件套**：`post-like.js` / `post-share.js` / `post-reward.js` 在 `post.astro` 各自按 `actionBar.like/share/reward` 子开关 `th:if` 门控；`qrcode.bundle.js` 不在模板引用，由 `post-share.js` 首次生成海报时经按钮 `data-qr-src` 动态注入（`window.__onlynnQRState` 守卫，加载失败走 hasQR 退化）。
