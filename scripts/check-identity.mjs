...
async function walk(dir, out = []) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (["node_modules", ".git", "templates", "dist"].includes(entry.name)) continue;
      await walk(full, out);
    } else {
      out.push(full);
    }
...
const themeName = /^metadata:\s*$[\s\S]*?^\s{2}name:\s*(\S+)/m.exec(themeYaml)?.[1];
...
if (!settingsYaml) {
  ...
  const declared = /^metadata:\s*$[\s\S]*?^\s{2}name:\s*(\S+)/m.exec(settingsYaml)?.[1];
...
const base = astroConfig ? /base:\s*[`"']([^`"']+)[`"']/.exec(astroConfig)?.[1] : null;
if (!base) {
  fail("astro.config.mjs 里找不到 base");
} else if (base !== `/themes/${themeName}`) {
  fail(`astro.config.mjs 的 base (${base}) 与主题名 (${themeName}) 不匹配，应为 /themes/${themeName}`);
}
...
for (const file of files) {
  ...
  for (const [, prefix] of content.matchAll(/\/themes\/([A-Za-z0-9_-]+)\//g)) {
    ...
    foreignPrefixes.set(prefix, [...(foreignPrefixes.get(prefix) ?? []), path.relative(root, file)]);
  }
}
for (const [prefix, where] of foreignPrefixes) {
  fail(`发现指向其它主题的资源前缀 /themes/${prefix}/ ，出现在：${[...new Set(where)].join(", ")}`);
}

// ── 5. annotation-setting.yaml 的扩展名应带本主题前缀 ────────────
const annotation = await readIfExists(path.join(root, "annotation-setting.yaml"));
if (annotation) {
  const name = /^metadata:\s*$[\s\S]*?^\s{2}name:\s*(\S+)/m.exec(annotation)?.[1];
...
// ── 6. 上游身份不应出现在对外位置 ─────────────────────────────────
const UPSTREAM = ["AloneNanNan", "halo-theme-ethereal"];
for (const file of files) {
  const rel = path.relative(root, file).split(path.sep).join("/");
  // README 与 LICENSE 需要保留归属声明，不检查
  if (/^(README|LICENSE|CHANGELOG|CONTRIBUTING)/.test(rel)) continue;
  const content = await readIfExists(file);
  if (!content) continue;
  for (const token of UPSTREAM) {
    if (content.includes(token)) {
      notes.push(`${rel} 仍含上游标识「${token}」—— 确认这是有意保留的归属声明`);
    }
  }
}
...
if (await stat(outIndex).then(() => true, () => false)) {
  ...
  if (!href) {
    ...
    const assetPath = path.join(root, "templates", href.replace(/^\/themes\/[^/]+\//, ""));
...
// ── 输出 ──────────────────────────────────────────────────────────
console.log(`主题身份：${themeName}  设置：${settingName}  配置：${configMapName}`);
